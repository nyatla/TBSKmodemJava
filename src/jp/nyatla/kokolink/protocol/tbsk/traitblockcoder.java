...
import jp.nyatla.kokolink.interfaces.IDecoder;
import jp.nyatla.kokolink.interfaces.IEncoder;
...
public class traitblockcoder
{
    ...
    // """
    static public class TraitBlockEncoder extends BasicRoStream<Double> implements IEncoder<TraitBlockEncoder,IBitStream, Double>{
        private int _pos;
        private final List<Integer> _sblock;
        ...
        // """
        public TraitBlockEncoder(TraitTone tone)
        {
        	super();
            this._sblock=new ArrayList<Integer>();
            this._sblock.add(-1);
            this._btone=tone;
            this._tone_q=new ArrayDeque<Double>();
        }
        ...
        public Double next() throws PyStopIteration
        {
            if(this._tone_q.size()==0){
                ...
                try{
                    ...
                    for(var i : this._sblock){
                        for(var j : this._btone){
                            this._tone_q.add(sign*i*j);
                        }
                    }
    ...
    public static class TraitBlockDecoder extends BasicRoStream<Integer> implements IBitStream,IDecoder<TraitBlockDecoder,IRoStream<Double>,Integer>
    {
        ...
        private int _shift;
        public TraitBlockDecoder(int trait_block_ticks) {
        	this(trait_block_ticks,0.2);
        }
        public TraitBlockDecoder(int trait_block_ticks,double threshold)
        {
            this._is_eos=true;
            this._trait_block_ticks=trait_block_ticks;
            this._threshold=threshold;
            this._samples = new ArrayList<Double>();// #観測値
        }
        ...
        public Integer next() throws PyStopIteration
        {
            ...
            try{
                ...
                var r=samples.get(1);
                if (samples.get(0) * samples.get(1) < 0 || samples.get(1) * samples.get(2) < 0) {
                    // #全ての相関値が同じ符号でなければ何もしない
                    this._block_skip_counter = this._block_skip_size; //#リセット
                } else {
                    var asample = new double[samples.size()];
                    // #全ての相関値が同じ符号
                    for (var i = 0; i < samples.size(); i++) {
                        asample[i] = Math.abs(samples.get(i));
                    }
                    // #一番大きかったインデクスを探す
